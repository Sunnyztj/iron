...
class jQuery_in_Posts_Pages
{
	...
	function wcp_script_cb(){
		...
		echo '<br><div><a target="_blank" href="http://codecanyon.net/item/custom-styles-and-scripts/14322786?ref=Rameez_Iqbal" class="button-secondary">Explore Pro Features</a></div>';		
	}

	function wcp_saving_custom_js($post_id){

		$wcpd_script = $_POST['wcp_custom_script'];
	...
	function enque_scripts_here(){
		...
		$wcp_meta = get_post_meta( $post->ID, 'wcp_custom_script', true );
		if ($wcp_meta != '') {
			echo '<script>jQuery(document).ready(function($) {'.stripcslashes($wcp_meta).'});</script>';		
		}		
	}
}
?>